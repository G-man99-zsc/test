...
function renderBellNotifications(list) {
    ...
    sorted.slice(0, 10).forEach(n => {
        ...
        li.innerHTML = `
            ...
            <div class="notif-actions">${renderNotificationReadButton(n)}</div>
        `;
        li.onclick = () => handleNotifClick(n.id);
        list.appendChild(li);
    });
...
function renderTaskNotificationsPanel() {
    ...
    list.innerHTML = sorted.map(n => `
        <div class="notification-item${isNotificationRead(n) ? "" : " unread"}" id="notif-${n.id}">
            ...
            <div class="notification-row-actions">${renderNotificationReadButton(n)}</div>
        </div>
    `).join("");
}
...
function startNotificationRealtime() {
    ...
    eventClient.connect({}, () => {
        eventClient.subscribe("/topic/events", (msg) => {
            ...
            const isTaskEvent = ["task_assigned", "task_completed_sound", "task_seen", "notifications_changed"].includes(event.event);
...
// end of profile page
// Finallyy, the profile page is DONE!!
